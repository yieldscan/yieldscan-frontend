...
const UnsupportedDeviceScreen = () => (<div className="bg-white">
	<main className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div className="max-w-xl mx-auto py-16 sm:py-24">
			<div className="text-center">
				<p className="text-sm font-semibold text-teal-600 uppercase tracking-wide">Device not supported</p>
				<h1 className="mt-2 text-4xl font-extrabold text-gray-900 tracking-tight sm:text-5xl">
					This page isn't supported on your device.
				</h1>
				<p className="mt-2 text-lg text-gray-600">The page you are looking for is not available on your device. To continue, please visit the page using a laptop or desktop computer.</p>
			</div>
			<div className="mt-12">
				<h2 className="text-sm font-semibold text-gray-600 tracking-wide uppercase">Helpful resources</h2>
						...
						<li key={linkIdx} className="relative py-6 flex items-start space-x-4">
							<div className="flex-shrink-0">
								<span className="flex items-center justify-center h-12 w-12 rounded-lg bg-teal-50">
									<link.icon className="h-6 w-6 text-teal-700" aria-hidden="true" />
								</span>
							</div>
...
const WithDashboardLayout = (children, isSetUp, isWalletSetUp) => {
	...
	return isMobile || isTablet ? (<UnsupportedDeviceScreen />) : (
		<div>
			...
			< div className="dashboard-content fixed flex relative w-full" >
				...
				<div
					className={`h-full ${!isSetUp && "px-8 mx-auto"
						} overflow-y-scroll  w-full`}
				>
					<div
						className={`mx-auto h-full ${isSetUp
							? "w-full"
							: includes(
