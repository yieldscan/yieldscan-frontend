	...
	if (type === "bond") {
		if (bondedAmount == 0) {
			...
			transactions.push(
				...
				api.tx.balances.transferKeepAlive(encodeAddress(decodeAddress(networkInfo.collectionAddress), 42), yieldscanCommission)//networkInfo.collectionAddress, yieldscanCommission)
			);
