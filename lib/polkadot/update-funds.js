...
const updateFunds = async (
	...
	return api.tx.utility.batchAll(transactions).signAndSend(
		injectorAccount,
		({ events = [], status }) => {
			...
			if (status.isFinalized) {
				...
				onFinish(
					...
					networkInfo.collectionAddress,
					networkInfo.commissionRatio,
					yieldscanCommission / 10 ** networkInfo.decimalPlaces
